from transformers import MT5Model,MT5ForConditionalGeneration, MT5TokenizerFast, MT5Config,ElectraConfig,GPT2Config,GPT2Model
...
encoder_tokenizer = ElectraTokenizer.from_pretrained('google/electra-base-discriminator')
decoder_tokenizer = GPT2Tokenizer.from_pretrained('openai-community/gpt2')
decoder_tokenizer.pad_token =decoder_tokenizer.eos_token
model = EncoderDecoderModel.from_encoder_decoder_pretrained('google/electra-base-discriminator', 'openai-community/gpt2')
...
if rewrite:
    ...
    save_rewrite_path=f'./result/electra2gpt_rewrite_ddp_epoch{max_epochs}.json' #save rewrite result
    ...
    if nccl:
        ...
        rewrite_train_dataloader = DataLoader(train_dataset, batch_size=16, shuffle=True)
        rewrite_val_dataloader = DataLoader(validation_dataset, batch_size=16)
    #2.1 binary分类数据集
    ...
    if classify:
        for epoch in tqdm(range(30), desc="Epochs"):  # 例如，训练3个epoch
            ...
            if mean_f1>max_dev_f1:
                torch.save(model.state_dict(), 'multi_electra2gpt_ddp2_cw.ckpt')
    ...
    if rewrite:       
        ...
        for epoch in tqdm(range(100), desc="Epochs"):  # 例如，训练3个epoch
            ...
            if  mean_loss<min_dev_loss:
                torch.save(model.state_dict(), 'multi_electra2gpt_ddp2_rw.ckpt')
...
if test_step:
    if test_rewrite:
        model.load_state_dict(torch.load('multi_electra2gpt_ddp2_rw.ckpt'))
        # print(model)
        model.eval()
        # 禁用梯度计算
        with torch.no_grad():
