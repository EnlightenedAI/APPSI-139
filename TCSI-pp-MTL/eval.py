...
def chinese_rouge_scorces(datafile_url):
    rouge = Rouge()
...
def rouge_scorces(datafile_url):
    rouge = Rouge()
    datas = open(datafile_url, 'r', encoding='utf-8').readlines()
    datas_all = []
    for line in datas:
        data = json.loads(line)
        datas_all.append(data)
    ...
    # 提取预测文本和参考文本，不需要分词
    hyps, refs = map(list, zip(*[
        [d['pred'] if len(d['pred']) != 0 else 'is null', 
         d['rewrite'] if len(d['rewrite']) != 0 else 'is null'] 
        for d in datas_all
    ]))
