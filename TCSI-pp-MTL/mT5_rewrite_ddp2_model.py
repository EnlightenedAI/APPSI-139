...
if rewrite:
    ...
    save_rewrite_path=f'./result/mt5_ddp2_epoch{max_epochs}.json' #save rewrite result
    ...
    test_dataset = FalseGenerationDataset(tokenizer,true_false_adjective_tuples_test, max_len_inp, max_len_out)
    if nccl:
        ...
        # rewrite_test_dataloader = DataLoader(test_dataset, batch_size=32)
    #2.1 binary分类数据集
    ...
    if classify:
        for epoch in tqdm(range(50), desc="Epochs"):  # 例如，训练3个epoch
            ...
            if mean_f1>max_dev_f1:
                torch.save(model.state_dict(), 'multi_mt5_ddp2_cw1.ckpt')
    ...
    if rewrite:    
        ...
        for epoch in tqdm(range(100), desc="Epochs"):  # 例如，训练3个epoch
            ...
            for batch in rewrite_train_dataloader:
                ...
                loss=model(
                    input_ids=batch["source_ids"].to(device),
                    attention_mask=batch["source_ids"].to(device),
                    labels=batch['labels'].to(device),
                    # decoder_input_ids=batch["target_ids"].to(device),
                    # decoder_attention_mask=batch['target_mask'].to(device),
                    task='rewrite'
            ...
            if  mean_loss<min_dev_loss:
                torch.save(model.state_dict(), 'multi_mt5_ddp2_rw1.ckpt')
        ...
        with torch.no_grad():
            with open(save_rewrite_path, 'w', encoding='utf-8') as fp:
                for text in true_false_adjective_tuples_test:
                    test_tokenized = tokenizer.encode_plus('summarization'+text[0][:250], return_tensors="pt")
