...
loss_fct = torch.nn.CrossEntropyLoss(ignore_index=50256)
...
encoder_tokenizer = AutoTokenizer.from_pretrained('microsoft/deberta-v3-base')
decoder_tokenizer = GPT2Tokenizer.from_pretrained('openai-community/gpt2')
decoder_tokenizer.pad_token =decoder_tokenizer.eos_token
model = EncoderDecoderModel.from_encoder_decoder_pretrained('microsoft/deberta-v3-base', 'openai-community/gpt2')
...
config = AutoTokenizer.from_pretrained('microsoft/deberta-v3-base')
...
if rewrite:
    ...
    save_rewrite_path=f'./result/debert2gpt_rewrite_ddp_epoch{max_epochs}.json' #save rewrite result
    ...
    if nccl:
        ...
        rewrite_train_dataloader = DataLoader(train_dataset, batch_size=8, shuffle=True)
        rewrite_val_dataloader = DataLoader(validation_dataset, batch_size=8)
    #2.1 binary分类数据集
...
if classify:
    ...
    important_train_iter,important_dev_iter,important_test_iter=binary_dataload(config,data_paths,nccl,batch_size=64)
    ...
    def multi_dataload(config,data_paths,nccl,batchsize=16):
        ...
        if nccl:
            ...
            # multi_train_iter = DataLoader(train_dataset,sampler=ImbalancedDatasetSampler(train_dataset), batch_size=batchsize)
    ...
    multi_train_iter,multi_dev_iter,multi_test_iter=multi_dataload(config,data_multi_paths,nccl,batchsize=32)
# 3. 定义训练参数
if nccl:
# 2. 创建模型并移动到对应的设备
...
# # 假设 model 是你的模型实例
# print(model)  # 确保 model 不是 None

# # 检查 model.parameters() 是否返回了参数列表
# # all_params = model.parameters()
# # print(all_params)  # 确保这不是 None

# # 如果 model 是 DataParallel 封装的模型，使用 model.module.parameters()
# if model.__class__.__name__ == 'DataParallel':
#     print('DataParallel封装')
...
#         print("模型没有参数或模型实例为空")
# else:
#     decoder_params = [p for p in all_params if p.requires_grad and 'decoder' in p.name]
# # 现在尝试过滤出 decoder 的参数
# decoder_params = [p for p in all_params if p.requires_grad and 'decoder' in p.name]

# 确保 decoder_params 不是 None
# print("decoder参数",decoder_params)

# decoder_params = [p for p in model.parameters() if p.requires_grad and 'decoder' in p.name]
...
if classify:
    ...
    optimizer_r = BertAdam(optimizer_grouped_parameters,
                            ...
                            # t_total=(len(multi_train_iter)) * num_epochs)


# optimizer_m = torch.# optimizer_m = torch.optim.AdamW(model.parameters(), lr=5e-6)
    ...
    if classify:
        for epoch in tqdm(range(30), desc="Epochs"):  # 例如，训练3个epoch
            ...
            if mean_f1>max_dev_f1:
                torch.save(model.state_dict(), 'multi_debert2gpt_ddp2_cw.ckpt')
                print(f'epoch{epoch}分类f1性能提升:',mean_f1)
                max_dev_f1=mean_f1
    if rewrite:        
        model.load_state_dict(torch.load('multi_debert2gpt_ddp2_cw.ckpt'))
        for epoch in tqdm(range(100), desc="Epochs"):  # 例如，训练3个epoch
            ...
            if  mean_loss<min_dev_loss:
                torch.save(model.state_dict(), 'multi_debert2gpt_ddp2_rw.ckpt')
...
if test_step:
    if test_rewrite:
        model.load_state_dict(torch.load('multi_debert2gpt_ddp2_rw.ckpt'))
        # print(model)
        model.eval()
        # 禁用梯度计算
        with torch.no_grad():
