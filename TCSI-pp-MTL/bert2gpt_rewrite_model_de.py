...
xlnet2gpt_model = EncoderDecoderModel.from_encoder_decoder_pretrained("google-bert/bert-base-cased", "openai-community/gpt2")
...
model.load_state_dict(torch.load('multi_bert2gpt_ddp2_cw.ckpt'))
...
savepath=f"/root/autodl-tmp/bert2gpt_ddp2_rewrite_epoch{num_epoch}.ckpt" #save model
save_rewrite_path=f'/root/autodl-tmp/bert2gpt_ddp2_rewrite_epoch{num_epoch}.json' #save rewrite result
...
if train_is_ture:
    ...
    # trainer = pl.Trainer(max_epochs = num_epoch,amp_level='01', gpus=1, progress_bar_refresh_rate=30,logger=False)
    trainer.fit(model)
