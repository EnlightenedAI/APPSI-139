...
from loss.focallooss import Focal_Loss_multi
import time
...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains_1, labels,lens,masks) in enumerate(train_iter):
            ...
            loss_func =Focal_Loss_multi(alpha=config.focalloss_rate, gamma=2)
            ...
            if total_batch % 1000 == 0:
                ...
                dev_acc, dev_loss,dev_pre,dev_rec,dev_f1= evaluate(config, model, dev_iter)
                improve=''
                ...
                if dev_acc > dev_best_acc:
                    ...
                    last_improve = total_batch
                # print(dev_f1)
                if np.mean(dev_f1) > dev_best_f1:
                    # print(np.mean(dev_f1))
...
def evaluate(config, model, data_iter, test=False):
    ...
    if test:
        ...
        report = metrics.classification_report(labels_all,predict_all,target_names=['数据采集', '共享披露','使用','存储方式','安全措施','特殊人群','权限管理','联系方式','停止运行'],digits=3)
