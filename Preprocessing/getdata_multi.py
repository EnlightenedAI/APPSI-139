...
# from .TCSI_pp.preprocessing.sklearnmulti import iterative_train_test_split # type: ignore
...
import numpy as np
dataset='test_data'
# path= 'TCSI_pp/preprocessing'+dataset+'/doccano/'
...
# datas_url='TCSI_pp/preprocessing/'+dataset+'/class.txt'
            ...
            if i <150 and file !='.DS_Store':
                ...
                with open(os.path.join(path_results, file), "r", encoding="utf-8") as f1:
                    for line in tqdm.tqdm(f1):
                        ...
                        # print(len(line_dict["label"]))
                        # if list(set( line_dict["label"]).intersection(set(['Important'])))==[] and len(line_dict["label"])>0:
